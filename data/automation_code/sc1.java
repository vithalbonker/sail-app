...
public class sc1 extends ConfigTestBase{
	...
	public void sc1() throws Exception{
        ...
        while (testScriptIterator.hasNext()){
            ...
            if(globalConstants.currentTSDataRowJSONObj.get("execute").toString().equalsIgnoreCase("yes")){			
				...
				headerParams.put("hp2",globalConstants.currentTSDataRowJSONObj.get("hp2").toString());
				queryParams.put("qp1",globalConstants.currentTSDataRowJSONObj.get("qp1").toString());
				pathParams.put("pp1",globalConstants.currentTSDataRowJSONObj.get("pp1").toString());
				...
				Assert.assertEquals(generic.verifyStatusCode(statusCode), Integer.parseInt("221"),"Service Call Error " + responseDetails);
				...
				Assert.assertTrue(getCallWrapper.verifyResponseContentType(response),"Response Content Type is not JSON" );
				appGeneric.generateExtentReport(GlobalConstants.ReportStatus.PASS.toString(), testScriptName);
